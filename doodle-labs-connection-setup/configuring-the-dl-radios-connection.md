...
[_**Bandwidth**_](https://en.wikipedia.org/wiki/Bandwidth) - The difference between the upper and lower frequencies used centered around the chanell frequency. The explanation is beyond the scope of this guide, however a few basic rules apply. A larger bandwidth can carry more information (regardless the frequency - if we dont account for higher losses along the path at higher frequencies). On the other hand a wider channel is more likely to overlap with neighbouring channels, increasing susceptibility to interference and packet loss. For the most reliable communication set the bandwidth as low as possible. If you wish to stream a lot of data at the same time and accept a higher loss, it is advantageous to set the bandwidth higher.
